...
Namespace soctdx2
    Module RetrieveSI
        ...
        Private Sub DirLoad(ByVal sender As Object, ByVal e As EventArgs)
            ...
            Dim tcSQL As String = String.Concat(New String() {"EXEC fs_GetSIDistr4Details '", modVoucher.cLan, "', '", Strings.Trim(modVoucher.strPRIDNumberx), "'"})
            ...
            AddHandler modVoucher.oDirFormLib.GetClsreports.GetGrid.GetGrid.CurrentCellChanged, New EventHandler(AddressOf RetrieveSI.grdMVCurrentCellChanged)
            AddHandler modVoucher.grdDVx.CurrentCellChanged, New EventHandler(AddressOf RetrieveSI.grdDVxCurrentCellChanged)
            AddHandler modVoucher.grdDVx.Enter, New EventHandler(AddressOf RetrieveSI.grdDVxCurrentCellChanged)
            RetrieveSI.RefreshDetail()
            ...
            modVoucher.oDirFormLib.GetClsreports.mnFile.MenuItems.Item(1).Text = StringType.FromObject(modVoucher.oLan.Item("610"))
            ...
            modVoucher.oDirFormLib.GetClsreports.tbr.Buttons.Item(1).ToolTipText = StringType.FromObject(modVoucher.oLan.Item("610"))
            ...
            AddHandler modVoucher.oDirFormLib.GetClsreports.GetGrid.GetButtonFilter.Click, New EventHandler(AddressOf RetrieveSI.cmdFilterClick)
            ...
            AddHandler modVoucher.iQty.TextBox.Leave, New EventHandler(AddressOf RetrieveSI.ColChanged)
            AddHandler modVoucher.inTransp.TextBox.Leave, New EventHandler(AddressOf RetrieveSI.ColChanged)
            modVoucher.oDirFormLib.GetClsreports.GetGrid.GetForm.Text = modVoucher.strTitle
        End Sub

        Private Sub FilterData()
            SIFilterData.SelectSI()
            RetrieveSI.GetPRHeader()
            modVoucher.dsDetailx.Clear()
        ...
        Private Sub GetPRHeader()
            Dim tcSQL As String = String.Concat(New String() {"EXEC fs_GetSIDetails4Distr '", modVoucher.cLan, "', '", Strings.Trim(modVoucher.strPRIDNumberx), "', 'ct81'"})
            oDirFormLib.GetClsreports.GetGrid.GetDataView.Table.Clear()
            Sql.SQLRetrieve((modVoucher.appConn), tcSQL, "wrksidetail", (oDirFormLib.GetClsreports.GetGrid.GetDataView.Table.DataSet))
        End Sub
        ...
        Public Sub RetrieveSIData()
            ...
            oDirFormLib.GetClsreports.strSQLRunReport = ("EXEC fs_GetSIDetails4Distr '" & modVoucher.cLan & "', '', 'ct81'")
            oDirFormLib.GetClsreports.strAliasReport = "wrksidetail"
            Dim frm As New frmE
            oDirFormLib.frmUpdate = frm
            AddHandler oDirFormLib.GetClsreports.GetGrid.GetForm.Load, New EventHandler(AddressOf RetrieveSI.DirLoad)
            oDirFormLib.Init()
            SIFilterData.SelectSI()
